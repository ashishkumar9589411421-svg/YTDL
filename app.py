...
import time
import threading
import secretsfrom flask import Flask, request, jsonify, send_file, send_from_directory
from flask_cors import CORS
import yt_dlp
import os
import shutil
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for f in info.get('formats', []):
                h = f.get('height')
                # If No FFmpeg, we can ONLY download single files.
                # Usually formats under 720p are single files.
                # If we show 1080p but can't merge, user gets error. 
                # So we list everything, but logic below handles the fallback.
        ...
        try:
            ...
            if fmt_id == "mp3":
                if HAS_FFMPEG:
                    ...
                    # Fallback: Download 'm4a' (audio only) directly if no FFmpeg to convert to MP3
                    print("⚠️ FFmpeg Missing: Downloading best available audio format (no conversion)")
            ...
            # MODE 2: VIDEO
                ...
                if HAS_FFMPEG:
                    # Best Case: We have FFmpeg -> Merge Video+Audio
                    common_opts["format"] = f"bestvideo[height<={height}]+bestaudio/best[height<={height}]/best"
                    common_opts["merge_output_format"] = "mp4"
                else:
                    # Worst Case: No FFmpeg -> FORCE 'best' single file
                    # This PREVENTS the "Requested format not available" error
                    print(f"⚠️ FFmpeg Missing: Cannot merge 1080p. Falling back to best single file (likely 720p).")
            ...
            with yt_dlp.YoutubeDL(common_opts) as ydl:
                ...
                for f in os.listdir(DOWNLOAD_FOLDER):
                    if f.startswith(job_id):
                        ...
                        found = True
                        break
                if not found: raise Exception("File missing after download")
